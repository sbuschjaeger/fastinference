...
def to_implementation(model, out_path, out_name, weight = 1.0, namespace = "FAST_INFERENCE", feature_type = "double", label_type = "double", float_type = "double", int_type = "signed int", uint_type = "unsigned int", infer_types = True, lut_size = 0, **kwargs):
    ...
    **Important**: This implementation does **not** perform any optimizations with respect to the HLS tool. We highly recommend to perform a design space exploration manually after the code generation for a given neural net to have the best performance. Additionally, the input data must likely be adapted manually to the given FPGA.
    ...
    You can use this implementation by simply passing :code:`"fpga.binary"` to implement:
    ...
    print("FEAUTER_TYPE {}".format(feature_type))
    is_first = True
    ...
    if lut_size > 1:
        lut = [bin(x).count("1") for x in range(1,(2**lut_size))] #compute popcount lookuptable
        lut_elem_size = len(bin(max(lut)))-2 #size of each element in lut
    else: 
        lut = [0]

    binary_word_size = infer_binary_wordsize(uint_type)

    #count_size = len(bin(weight_bit_size))-2 #return size of popcount function
    #count_size = len(bin(binary_word_size))-2 #return size of popcount function
    popcount_impl = env.get_template(os.path.join("popcount.j2")).render(
        lut=lut,
        lut_elem_size=lut_elem_size if lut_size > 0 else '', 
        #n=weight_bit_size, 
        n=binary_word_size, 
        m=lut_size
    )

    code_static = popcount_impl + "\n" + code_init + "\n" + code_alloc
