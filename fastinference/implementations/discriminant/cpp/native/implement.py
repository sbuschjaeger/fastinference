...
def to_implementation(model, out_path, out_name, weight = 1.0, namespace = "FAST_INFERENCE", feature_type = "double", label_type = "double", internal_type = "double",**kwargs ):
    """Generates a (unrolled) C++ implementation of the given DiscriminantAnalysis model. Unrolled means that parameters of the model are not stored in arrays but are directly translated into the source code. You can use this implementation by simply passing :code:`"cpp"` to the implement, e.g.
    ...
    Args:
        model (DiscriminantAnalysis): The DiscriminantAnalysis model to be implemented
